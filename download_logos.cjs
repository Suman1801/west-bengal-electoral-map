...
const promises = Object.entries(logos).map(([party, url]) => {
  return new Promise((resolve, reject) => {
    ...
    https.get(url, { headers: {'User-Agent': 'Mozilla/5.0 (Node.js)'} }, function(response) {
      if (response.statusCode === 301 || response.statusCode === 302) {
         https.get(response.headers.location, { headers: {'User-Agent': 'Mozilla/5.0 (Node.js)'} }, function(res2) {
             res2.pipe(file);
